...
export class ConnectionController {
  ...
  async createFromSwap(swapId: string) {
    ...
    const swap = await db.get(
      `SELECT sw.requester_id, sw.responder_id, l.title
       FROM swaps sw JOIN listings l ON sw.target_skill_id = l.id
       WHERE sw.id = ?`,
      swapId
    );
    if (!swap) return;
    ...
    await db.run(
      `INSERT INTO connections (id, user1_id, user2_id, skill_title, source_type, source_id)
       VALUES (?, ?, ?, ?, 'swap', ?)`,
      [randomUUID(), swap.requester_id, swap.responder_id, swap.title, swapId]
    );
  }
