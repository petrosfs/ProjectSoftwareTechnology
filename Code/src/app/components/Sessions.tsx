...
function ScheduleModal({
  ...
  // Fetch available skills based on role and selected user
  useEffect(() => {
    setSkillTitle('');
    setAvailableSkills([]);
    if (myRole === 'teaching') {
      ...
      fetch('/api/myskills', { credentials: 'include' })
        .then(r => r.ok ? r.json() : [])
        .then((data: any[]) => setAvailableSkills(Array.isArray(data) ? data.map(s => ({ id: s.id, name: s.name })) : []))
        .finally(() => setLoadingSkills(false));
    } else if (myRole === 'learning' && selectedUser) {
      setLoadingSkills(true);
      fetch(`/api/users/${selectedUser.id}/skills`, { credentials: 'include' })
        .then(r => r.ok ? r.json() : [])
        .then((data: any[]) => setAvailableSkills(Array.isArray(data) ? data.map(s => ({ id: s.id, name: s.name })) : []))
        .finally(() => setLoadingSkills(false));
    }
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          <div>
            ...
            {loadingSkills ? (
              <div className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm text-gray-400 flex items-center gap-2">
              ...
              <div className="w-full px-4 py-2.5 border border-gray-100 rounded-xl text-sm text-gray-400 bg-gray-50">
                {myRole === 'learning' && !selectedUser ? 'Select a teacher first' : 'No skills available'}
              </div>
            )}
          </div>
